...
def get_nsa_decals(highz=True):
    ...
    nsa_decals = fits.getdata('%s/decals/fits/nsa_decals_v%s.fits' % (gzpath,version),1)
...
def plot_hists(highz=True):
    ...
    plt.savefig('%s/decals/plots/nsa_decals_hist_v%s.pdf' % (gzpath,version))

    return None

def get_tractor(brickname='0001m002'):

    tractorfile = '%s/decals/fits/tractor-%s.fits' % (gzpath,brickname)
...
def imageserver_fits(ra=114.5970,dec=21.5681,pixscale=2.,size=424):
    ...
    urllib.urlretrieve(url, "%s/decals/fits/images/%s.fits" % (gzpath,'decals'))
...
def check_fits_badpixels():
    ...
    nsa_decals = fits.getdata('%s/decals/fits/nsa_decals_v1_0_0.fits' % gzpath,0)
    N = 50
    ...
    # Import into Python
        ...
        try:
            img,hdr = fits.getdata("%s/decals/fits/images/%s.fits" % (gzpath,"decals"),0,header=True)
...
def test_images_etgs(public=False):
    ...
    galaxies = fits.getdata('%s/decals/fits/stripe82_etgs_decals.fits' % gzpath,1)
